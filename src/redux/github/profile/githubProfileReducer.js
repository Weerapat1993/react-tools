  ...
  switch(action.type) {
    case FETCH_GITHUB_PROFILE.REQUEST:
      return reducer.setState({
        keys: reducerKeys.getRequestWithKey(action.key)
      })
    ...
    case FETCH_GITHUB_PROFILE.SUCCESS:
      return reducer.setState({
        ...
        keys: reducerKeys.getSuccessWithKey(action.key, { data: action.data })
      })
      // return reducer.getSuccess({ data: action.data })
    case FETCH_GITHUB_PROFILE.FAILURE:
    return reducer.setState({
      keys: reducerKeys.getFailureWithKey(action.key)
    })
